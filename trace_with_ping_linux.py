...
def traceroute_rhel(hostname):
		print('traceroute to %s, 64 hops max, ~60 byte packets' % (hostname))
		for i in range(0, 64):
			start_time = time.time()
			ping = subprocess.Popen(["ping", "-c", "1", "-W", "1", "-t", str(i), hostname], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			end_time = time.time()
